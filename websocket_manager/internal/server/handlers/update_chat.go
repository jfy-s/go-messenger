...
func HandleUpdateChat(storage storage.Storage, msgPacketRequest *model.MessagePacketRequest, logger *slog.Logger) *model.MessagePacketRequest {
	req := UpdateChatRequest{CreatorID: msgPacketRequest.From, ChatID: msgPacketRequest.To, Name: msgPacketRequest.Data}
	validator := validator.New()
	if err := validator.Struct(req); err != nil {
		logger.Error("failed to validate request", "error", err)
		return &model.MessagePacketRequest{MsgType: model.UpdateChat, From: 0, To: msgPacketRequest.From, Data: "Internal Error"}
	}
	...
	return &model.MessagePacketRequest{MsgType: model.UpdateChat, From: 0, To: msgPacketRequest.From, Data: "Success"}
}
